...
use \Google_Service_Webmasters as Google_Service_Webmasters;
use \Google_Service_Analytics_Webproperty as Google_Service_Analytics_Webproperty;
use \Google_Service_Webmasters_SearchAnalyticsQueryRequest as Google_Service_Webmasters_SearchAnalyticsQueryRequest;
use \Google_Service_Webmasters_SearchAnalyticsQueryResponse as Google_Service_Webmasters_SearchAnalyticsQueryResponse;
use \Google_Service_Webmasters_ApiDataRow as Google_Service_Webmasters_ApiDataRow;
use \Google_Service_Exception as Google_Service_Exception;
use \Google_Service_Webmasters_SitesListResponse as Google_Service_Webmasters_SitesListResponse;
use \Google_Service_GoogleAnalyticsAdmin as Google_Service_GoogleAnalyticsAdmin;
use \Google_Service_AnalyticsData as Google_Service_AnalyticsData;
use \Google_Service_AnalyticsData_RunReportRequest as Google_Service_AnalyticsData_RunReportRequest;
use \Google_Service_AnalyticsData_DateRange as Google_Service_AnalyticsData_DateRange;
use \Google_Service_AnalyticsData_Metric as Google_Service_AnalyticsData_Metric;
use \Google_Service_AnalyticsData_Dimension as Google_Service_AnalyticsData_Dimension;
use \Google_Service_AnalyticsData_Entity as Google_Service_AnalyticsData_Entity;
use \Google_Service_AnalyticsData_Filter as Google_Service_AnalyticsData_Filter;
use \Google_Service_AnalyticsData_FilterExpression as Google_Service_AnalyticsData_FilterExpression;
use \Google_Service_AnalyticsData_StringFilter as Google_Service_AnalyticsData_StringFilter;
use \Google_Service_AnalyticsData_BatchRunReportsRequest as Google_Service_AnalyticsData_BatchRunReportsRequest;
use \Google_Service_AnalyticsReporting as Google_Service_AnalyticsReporting;
use \Google_Service_AnalyticsReporting_DateRange as Google_Service_AnalyticsReporting_DateRange;
use \Google_Service_AnalyticsReporting_Metric as Google_Service_AnalyticsReporting_Metric;
use \Google_Service_AnalyticsReporting_Dimension as Google_Service_AnalyticsReporting_Dimension;
use \Google_Service_AnalyticsReporting_ReportRequest as Google_Service_AnalyticsReporting_ReportRequest;
use \Google_Service_AnalyticsReporting_DimensionFilter as Google_Service_AnalyticsReporting_DimensionFilter;
use \Google_Service_AnalyticsReporting_DimensionFilterClause as Google_Service_AnalyticsReporting_DimensionFilterClause;
use \Google_Service_AnalyticsReporting_GetReportsRequest as Google_Service_AnalyticsReporting_GetReportsRequest;
use \Google_Task_Runner as Runner;
use \Google_Http_Batch as Google_Http_Batch;
...
class Ahrefs_Seo_Analytics extends Ahrefs_Seo_Abstract_Api {
	...
	public function __construct() {
		$this->api_user           = substr( 'w' . implode( '-', [ get_current_user_id(), get_current_blog_id(), wp_parse_url( get_site_url(), PHP_URL_HOST ) ?? '' ] ), 0, 40 );
	 ...
	 * @return bool
	 */
	public function is_token_set() : bool {
	...
	private function get_token_scope_as_string() : string {
		if ( ! empty( $this->token ) ) {
	 ...
	 */
	private function gsc_check_domain() : bool {
	 ...
	 * @return string
	 */
	public function get_ua_id() : string {
		return $this->ua_id;
	}
	 ...
	 * @return string
	 */
	public function get_gsc_site() : string {
	 ...
	 * @return string
	 */
	public function get_ua_name() : string {
		return $this->ua_name;
	}
	 ...
	 * @return string
	 */
	public function get_ua_url() : string {
		return $this->ua_url;
	}
	 ...
	 * @return void
	 */
	private function tokens_save( $token ) : void {
	...
	private function tokens_load() : void {
		...
		if ( $prev_value !== $data ) {
			...
			$this->ua_name  = $data['ua_name'] ?? '';
			$this->ua_url   = $data['ua_url'] ?? '';
			$this->gsc_site = $data['gsc_site'] ?? '';
			$prev_value     = $data;
		}
	}
	...
	protected function load_accounts_list_ga4() : array {
		...
		try {
			...
			do {
				$params = [
					'pageSize' => $this::QUERY_LIST_GA_ACCOUNTS_PAGE_SIZE,
				];
				if ( ! empty( $next_list ) ) {
			...
			foreach ( $response_total->getRows() as $row ) {
				if ( $row instanceof Google_Service_Webmasters_ApiDataRow ) {
					if ( $row->getClicks() > 0 ) { // use only pages with traffic > 0.
						// key[0] is a page url.
						$result[] = wp_parse_url( $row->getKeys()[0] ?? '', PHP_URL_PATH ) ?? '';
					}
				}
			}
		}
	 ...
	 */
	private function get_found_pages_by_ua_id_ga( array $ua_ids, string $start_date, string $end_date ) : array {
		$results = [];
		try {
			$client = $this->create_client();
			$client->setUseBatch( true );
	 ...
	 * @return array<string, null|int> Array, [ ua_id => pages_found ].
	 */
	public function get_found_pages_by_ua_id_ga4( array $ua_ids, string $start_date, string $end_date ) : array {
		$results = [];
		try {
			$client = $this->create_client();
			$client->setUseBatch( true );
				...
				if ( is_array( $errors ) && ( 0 < count( $errors ) ) && isset( $errors[0]['reason'] ) ) {
					...
					if ( 'forbidden' === $reason ) {
						if ( $is_gsc && $this->gsc_site ) { // if was not disconnected before.
							$site    = preg_match( "/site '([^']+)'/", $errors[0]['message'] ?? $e->getMessage(), $m ) ? $m[1] : $this->gsc_site; // get site from message.
	 ...
	 * @return void
	 */
	private function prepare_gsc_query( string $key, Google_Http_Batch &$batch, Google_Service_Webmasters $service_webmasters, string $gsc_site, array $parameters ) : void {
		$result = [];
		try {
			$post_body = new Google_Service_Webmasters_SearchAnalyticsQueryRequest( $parameters );
	 ...
	 */
	protected function parse_gsc_response( ?Google_Service_Webmasters_SearchAnalyticsQueryResponse $response ) : array {
		$result = [];
		if ( ! empty( $response ) ) {
			/**
			* @var \Google_Service_Webmasters_ApiDataRow $row
	 ...
	 */
	public function get_clicks_and_impressions_by_urls( array $urls, string $start_date = null, string $end_date = null, ?int $limit = null, bool $without_totals = false, ?array $current_keywords = null ) : ?array {
		// is GSC enabled?
		if ( ! $this->is_gsc_enabled() ) {
			$this->set_message( 'Google Search Console disconnected.' );
			$this->service_error = [ [ 'reason' => 'internal-no-token' ] ];
		...
		foreach ( $urls as $key => $url ) {
			...
			$current_keyword       = $current_keywords[ $key ] ?? null;
	 ...
	 */
	public function get_position_fast( array $url_with_keyword ) : ?array {
		Ahrefs_Seo::breadcrumbs( __METHOD__ . wp_json_encode( func_get_args() ) );
		$result     = null;
	 ...
	 */
	public function get_gsc_disconnect_reason() : ?Message {
	 ...
	 * @return void
	 */
	public function set_gsc_disconnect_reason( ?string $string = null, ?Message $message = null, bool $reset_gsc_account = true ) : void {
	...
	protected function extract_message( \Exception $e, ?string $default_message = null, bool $skip_disconnected_message = true ) : ?string {
		...
		if ( $e instanceof Google_Service_Exception ) {
			...
			if ( is_array( $errors ) && count( $errors ) && isset( $errors[0]['message'] ) && isset( $errors[0]['reason'] ) ) {
				if ( $skip_disconnected_message && in_array( $errors[0]['reason'], [ 'userRateLimitExceeded', 'rateLimitExceeded', 'quotaExceeded', 'internalError', 'forbidden' ], true ) ) { /** @see Worker_Any::on_rate_error() */
