...
<div class="new-token-button">
	...
	<select class="account" name="gsc_site" id="gsc_account">
		<option value="" <?php selected( $gsc_site, '', true ); ?>>Please select</option>
		<?php
		$last_account = '';
		...
		foreach ( $gsc_list as $id => $item ) {
			...
			?>
			<option value="<?php echo esc_attr( $gsc_site_current ); ?>" <?php selected( $gsc_site_current, $gsc_site ); ?> <?php disabled( 'siteUnverifiedUser', $item['level'] ?? '' ); ?>><?php echo esc_html( $title ); ?></option>
			<?php
		}
		?>
	</select>
