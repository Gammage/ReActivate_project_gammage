...
class Ahrefs_Seo_Noindex extends Ahrefs_Seo_Abstract_Api {
	 ...
	 */
	public function is_noindex( array $page_id_to_url_list ) : array {
	 ...
	 */
	public function load_pages( array $page_id_to_url_list ) : array {
		$results = [];
		try {
			$options = [
	 ...
	 * @return array Associave array post_id => int: 0 - index, 1 - noindex, -1 - error.
	 */
	public function detect_is_noindex( array $pages_details, array $page_id_to_url_list ) : array {
		$results = [];
		foreach ( $pages_details as $post_id => $values ) {
			$link = $page_id_to_url_list[ $post_id ] ?? "post ID $post_id";
			...
			if ( ! $values['success'] ) {
				...
				// check content.
				if ( ! $is_noindex ) {
					$is_noindex = $this->check_html_meta( $values['body'] ?? '' );
				}
	 ...
	 */
	protected function check_header_x_robots_tag( array $headers ) : bool {
	 ...
	 * @return bool True if is noindex.
	 */
	protected function check_html_meta( string $body ) : bool {
