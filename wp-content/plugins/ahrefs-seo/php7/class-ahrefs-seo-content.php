...
class Ahrefs_Seo_Content {
	 ...
	 */
	public function get_waiting_weeks() : int {
	 ...
	 * @return string[]|null
	 */
	public function get_pages_checked() : ?array {
	 ...
	 * @return string[]|null
	 */
	public function get_posts_categories_checked() : ?array {
		$result = get_option( self::OPTION_POSTS_CAT, null );
		return is_array( $result ) ? $result : null;
	}

	public function is_disabled_for_pages() : bool {
		$value = get_option( self::OPTION_DISABLED_PAGES );
		return ! empty( $value );
	}

	public function is_disabled_for_posts() : bool {
		$value = get_option( self::OPTION_DISABLED_POSTS );
		return ! empty( $value );
	}

	protected function set_waiting_weeks( int $waiting_weeks ) : void {
		update_option( self::OPTION_WAITING_WEEKS, max( 1, $waiting_weeks ) );
	}

	protected function set_disabled_for( bool $pages_disabled, bool $posts_disabled ) : void {
	 ...
	 * @return void
	 */
	protected function set_pages_checked( ?array $values ) : void {
		if ( ! is_null( $values ) ) {
	 ...
	 * @return void
	 */
	protected function set_posts_categories_checked( array $values ) : void {
	 ...
	 * @return void
	 */
	protected function pages_add_to_checked( array $post_ids ) : void {
		$enabled = $this->get_pages_checked() ?? [];
	 ...
	 * @return void
	 */
	protected function pages_remove_from_checked( array $post_ids ) : void {
		$enabled = $this->get_pages_checked() ?? [];
