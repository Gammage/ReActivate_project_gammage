...
class Ahrefs_Seo_Compatibility {
	...
	public static function quick_compatibility_check() : bool {
		...
		if ( ! empty( $incorrect_classes ) && class_exists( '\\ReflectionClass' ) ) {
			...
			$result             = self::return_error_message( self::$plugins_list, self::$themes_list, self::$files_list, $error ) ?? $error; // alternative error message if no source detected, but incompatibility exists.
	...
	private static function analyze_stack( array $trace, string $method, string $file ) : array {
		...
		foreach ( $trace as $item ) {
			if ( isset( $item['file'] ) ) {
				...
				if ( isset( $item['class'] ) &&
					( $file === $path && $method === $item['class'] . '::' . $item['function'] // place where Error catched.
			...
			if ( is_array( $plugins_found ) && count( $plugins_found ) ) {
				...
				array_walk(
					...
					function( $plugin_slug ) use ( &$plugins_list ) {
						...
						$plugins_list[] = ( $fields['Name'] ?? $plugin_slug ) . ( ! empty( $fields['Version'] ) ? sprintf( ' version %s', $fields['Version'] ) : '' );
					}
				);
				return;
			}
	...
	protected static function return_error_message( $plugins_list, $themes_list, $files_list, $common_reason ) : ?string {
		...
		if ( count( $plugins_list ) ) {
			...
			$result       = sprintf(
				_n(
					'We’ve scanned your WordPress environment and discovered %s which is conflicting with the Ahrefs SEO plugin. Please pause the plugin by deactivating it before running content audit again.',
					'We’ve scanned your WordPress environment and discovered %s which are conflicting with the Ahrefs SEO plugin. Please pause these plugins by deactivating them before running content audit again.',
					count( $plugins_list )
				),
			...
			$result = sprintf(
				_n(
					'File %s has code incompatible with Ahrefs SEO plugin.',
					'One of files %s have code incompatible with Ahrefs SEO plugin.',
					count( $files_list )
				),
				implode( ',', $files_list )
			);
