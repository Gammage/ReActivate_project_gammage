...
class Worker_Traffic extends Worker_Any {
	...
	public function update_posts_info( array $post_ids ) : void {
		...
		$traffic_results = ! empty( $traffic_raw ) || ! empty( $skipped_results ) ? $this->calculate_traffic( array_merge( $skipped_results ?? [], $traffic_raw ?? [] ), $page_id_to_slug ) : [];
	 ...
	 */
	public function calculate_traffic( array $traffic_details, array $post_id_to_slug ) : array {
		$results         = [];
		$slug_to_post_id = array_flip( $post_id_to_slug );

		foreach ( $traffic_details as $slug => $values ) {
			$post_id = $slug_to_post_id[ $slug ] ?? null;
			...
			if ( ! is_null( $post_id ) ) {
				...
				if ( empty( $error ) ) {
					...
					$organic       = $values['Organic Search'] ?? 0;
	...
	protected function update_traffic_values( array $results ) : void {
		...
		foreach ( $results as $post_id => $values ) {
			try {
				$this->content_audit->update_traffic_values( (int) $post_id, $values['total'], $values['organic'], $values['total_month'], $values['organic_month'], $values['error'] ?? null );
			} catch ( \Error $e ) {
