...
abstract class Message {
	...
	public function __construct( array $message_fields ) {
		$this->type    = $message_fields['type'] ?? 'error';
		$this->classes = [];
	...
	public static function account_disconnected( bool $ahrefs_disconnected, bool $google_disconnected ) : self {
		...
		return self::create(
			[
				...
				'message' => 'The plugin needs Ahrefs, GSC and GA accounts to be connected to run content audits since content suggestion are based on these data. '
				. 'You can check your connected accounts in the settings.',
				'buttons' => $buttons,
			]
		);
	}
	...
	public static function ahrefs_limited() : self {
		...
		return self::create(
			[
				...
				'message' => sprintf(
					'Your Ahrefs account has been disconnected because you have used up all monthly integration rows available on your %s plan. '
					. 'Please consider upgrading your plan now or wait until your limits reset. The report will not be updated till the limits are reset.',
					esc_html( $plan )
				),
				'buttons' => [ 'ahrefs' ],
			]
