...
class Keywords_Tokenizer {
	 ...
	 * @return array<mixed>
	 */
	public function tokenize( string $text, int $words_count = 5, bool $return_array = false ) : array {
			...
			if ( '' !== $part ) {
				...
				foreach ( $substrings as $b => $word ) {
					...
					if ( $queue_len === $word_len ) { // add phrases from queue to result.
						...
						$this->add_phrase( $result, $word_len, $index, $a, $b - 1, true ); // b-1 because of queue length = 2 and we need index of starting word.
					}
				}
			}
		}
		...
		for ( $word_len = 3; $word_len <= $words_count; $word_len++ ) {
			...
			if ( count( $existing_phrases ) ) {
				...
				foreach ( $existing_phrases as &$pointer ) {
					$a         = &$pointer[0];
					$b_current = &$pointer[1];
					// can we add a word before the currect phrase to new phrase?
					if ( $b_current > 0 ) {
						$b     = $b_current - 1;
						$index = implode( ' ', array_slice( $content_blocks[ $a ], $b, $word_len ) );
						$this->add_phrase( $result, $word_len, $index, $a, $b );
					}
					// can we add a word after the currect phrase to new phrase?
	...
	private function add_phrase( array &$result, int $word_len, string &$index, int $a, int $b, bool $force_add = false ) : void {
		if ( isset( $this->counters[ $word_len ][ $index ] ) ) {
			// check is $a $b pair unique?
			if ( in_array( [ $a, $b ], $this->counters[ $word_len ][ $index ][1] ) ) {
	...
	private function is_stop_word( string &$token ) : bool {
	 ...
	 *
	 * @return array<string, int> Asociative array where word is index.
	 */
