...
class Ahrefs_Seo_Keywords {
	 ...
	 */
	public function post_keyword_get( int $snapshot_id, int $post_id ) : ?string {
		global $wpdb;
	...
	public function post_keyword_manual_get( int $snapshot_id, int $post_id ) : ?string {
		global $wpdb;
	 ...
	 * @return bool
	 */
	public function post_keywords_need_update( int $snapshot_id, int $post_id ) : bool {
		global $wpdb;
	 ...
	 * @return bool
	 */
	public function post_keywords_is_approved( int $snapshot_id, int $post_id ) : bool {
		global $wpdb;
	 ...
	 * @return void
	 */
	public function post_keywords_set_updated( int $snapshot_id, int $post_id ) : void {
		global $wpdb;
	 ...
	 * @return bool
	 */
	public function post_keywords_set( int $snapshot_id, int $post_id, ?string $keyword, ?string $keyword_manual = null, bool $reanalyze_everything = true ) : bool {
		global $wpdb;
	 ...
	 * @return void
	 */
	private function maybe_add_post_and_retry_query( int $snapshot_id, int $post_id, string $sql ) : void {
		global $wpdb;
	 ...
	 */
	private function create_single_post_keywords( int $post_id, int $keywords_limit = 50 ) : ?array {
	 ...
	 */
	private function create_posts_keywords( array $posts_ids, int $keywords_limit = 10 ) : array {
		$result = [];
		...
		foreach ( $posts_ids as $post_id ) {
			$result[ $post_id ] = $this->all_keywords[ $post_id ] ?? null;
		}
		return $result;
	}
	 ...
	 * @return void
	 */
	public function update_position_cache( int $snapshot_id, int $post_id, array $row ) : void {
		$cached = $this->get_cached_detail_for_posts( $snapshot_id, [ $post_id ] );

		if ( is_array( $cached[ $post_id ] ) ) {
			$kw_gsc = $cached[ $post_id ]['keywords'] ?? [];
	 ...
	 */
	protected function get_cached_detail_for_posts( int $snapshot_id, array $post_ids ) : array {
		$results = [];
	 ...
	 */
	private function get_cached_suggestions( int $snapshot_id, int $post_id ) : array {
		global $wpdb;
		...
		if ( ! empty( $fields ) ) {
			return [
				is_null( $fields['kw_gsc'] ) ? null : json_decode( $fields['kw_gsc'], true ),
				is_null( $fields['kw_idf'] ) ? null : json_decode( $fields['kw_idf'], true ),
			];
		}
		return [ null, null ];
	}
	 ...
	 * @return void
	 */
	private function set_cached_suggestions( int $snapshot_id, int $post_id, ?array $keywords, ?array $keywords2 ) : void {
		global $wpdb;
	 ...
	 */
	public function get_full_detail_for_posts( int $snapshot_id, array $post_ids, ?int $limit = null, bool $without_totals = false, $gsc_only = false ) : array {
		...
		foreach ( $post_ids as $post_id ) {
			...
			if ( 'publish' === get_post_status( $post_id ) ) { // only for existing and published posts.
				...
				$this->set_cached_suggestions( $snapshot_id, $post_id, $result['keywords'] ?? null, $result['keywords2'] ?? null );
				$result['url'] = $url;
			} else {
		...
		if ( ! empty( $requests ) ) {
			...
			foreach ( $requests as $post_id => $url ) {
				$results[ $post_id ]['keywords'] = is_array( $clicks ) ? ( $clicks[ $post_id ] ?? null ) : null;
				...
				$this->set_cached_suggestions( $snapshot_id, $post_id, $results[ $post_id ]['keywords'] ?? null, $results[ $post_id ]['keywords2'] ?? null );
			}
		}
		return $results;
	}
	 ...
	 * @return void
	 */
	private function fill_using_cached_gsc_results( array $cached_results, array &$keyword_row ) : void {
	 ...
	 */
	public function get_suggestions( int $snapshot_id, int $post_id, bool $use_cached_data = false ) : array {
		$errors           = null;
		$keywords         = [];
		...
		$data             = $data[ $post_id ] ?? null;
		$source_gsc       = false; // source of report is GSC data.
		$total_clicks     = 0;
		$total_impr       = 0;
		$keyword_manual   = $data['keyword_manual'] ?? null;
		$cached_positions = [];
		...
		$item = [ ! $item_selected && ! is_null( $keyword_manual ) && 0 === strcasecmp( $keyword_manual, $keyword ), $keyword_manual ?? '', 'manual', null, '-', '-' ];
		$this->fill_using_cached_gsc_results( $cached_positions, $item );
	 ...
	 * @return void
	 */
	public function get_time_period( string &$start_date, string &$end_date ) : void {
	 ...
	 */
	public function load_position_value_fast( int $snapshot_id, array $url_with_keyword ) : ?array {
	 ...
	 */
	public function load_position_value( int $snapshot_id, array $post_with_keyword ) : ?array {
		$result   = null;
	 ...
	 */
	public function load_position_from_cache( int $snapshot_id, int $post_id, string $current_keyword ) : ?float {
		$result                  = null;
