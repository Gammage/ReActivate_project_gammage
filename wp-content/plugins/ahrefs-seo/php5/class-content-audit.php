...
class Content_Audit {
	...
	const TRANSIENT_NAME      = 'ahrefs-content-running7';
	const TRANSIENT_NAME_CRON = 'ahrefs-content-running0';
	/** Max time allowed for single content update (seconds). Will exit after this time end. */
	const MAX_UPDATE_TIME = 15;
	/** Is audit stopped? */
	const OPTION_AUDIT_STOPPED = 'ahrefs-seo-audit-stopped';
	/** Reason, why it is not possible to run audit. */
	const OPTION_AUDIT_STOP_REASON           = 'ahrefs-seo-audit-stop-reason';
	const OPTION_AUDIT_STOP_REASON_SCHEDULED = 'ahrefs-seo-audit-stop-reason-scheduled';
	/**
	 * @var Snapshot
	 */
	protected $snapshot;
	 ...
	 */
	public function update_table( $run_from_cron = false ) {
					...
					if ( ! $initiated ) {
						...
						if ( $this->has_unprocessed_items() ) {
							...
							while ( $has_more_items && ! Ahrefs_Seo::should_finish() && ! $this->maybe_finish( $time ) ) {
								...
								if ( ! $has_more_items && ! is_null( $this->workers_waiting_time ) && ! Ahrefs_Seo::should_finish( intval( $this->workers_waiting_time + 10 ) ) && time() - $time + $this->workers_waiting_time <= self::MAX_UPDATE_TIME + 5 ) {
									$has_more_items = true;
	 ...
	 * @return void
	 */
	public function audit_delete_post_details( $post_id ) {
		global $wpdb;
		if ( ! is_null( $this->snapshot_id ) ) {
			$wpdb->delete(
				$wpdb->ahrefs_content,
	 ...
	 */
	protected function maybe_finish( $initial_time ) {
	 ...
	 */
	public function has_unprocessed_items() {
		global $wpdb;
		$additional_where   = [];
	 ...
	 */
	public function content_get_unprocessed_percent() {
		global $wpdb;
		if ( is_null( $this->snapshot_id ) ) {
			return 0;
		}
	 ...
	 * @return void
	 */
	private function init_item( $post_id, $snapshot_id, $new_action ) {
		global $wpdb;
	 ...
	 */
	protected function update_initial_statuses() {
		global $wpdb;
		Ahrefs_Seo::breadcrumbs( __METHOD__ );
	 ...
	 */
	public function get_unprocessed_item_from_new( $what_to_return = null ) {
		global $wpdb;
		$result           = [];
		$additional_where = [];
	 ...
	 */
	protected function get_unprocessed_item_id_from_prepared() {
		global $wpdb;
		// update "out of scope" or "analyzing" items with error or noindex status.
		$result = $wpdb->get_var( $wpdb->prepare( "SELECT post_id FROM {$wpdb->ahrefs_content} WHERE snapshot_id = %d AND ( action = %s OR action = %s ) AND ( total < 0 OR organic < 0 OR backlinks < 0 OR position < 0 OR is_noindex < 0 OR is_noindex = 1 ) LIMIT 1", $this->snapshot_id, Ahrefs_Seo_Data_Content::ACTION4_ANALYZING, Ahrefs_Seo_Data_Content::ACTION4_OUT_OF_SCOPE_ANALYZING ) );
		if ( is_null( $result ) ) {
			// "analyzing" items.
	 ...
	 * @return int|null post id or null.
	 */
	protected function get_unprocessed_item_id_from_finished() {
		global $wpdb;
	 ...
	 */
	protected function set_recommended_action( $post_id, $set_only_inactive_items = false ) {
		global $wpdb;
	 ...
	 * @return void
	 */
	public function update_post_info_position( array $post_ids, $fast_update = false ) {
		Ahrefs_Seo::breadcrumbs( __METHOD__ . wp_json_encode( func_get_args() ) );
	 ...
	 * @return void
	 */
	public function update_traffic_values( $post_id, $total = null, $organic, $total_month = null, $organic_month, $error = null ) {
		global $wpdb;
		if ( ! is_null( $total ) ) {
	 ...
	 * @return void
	 */
	public function update_backlinks_values( $post_id, $backlinks, $error = null ) {
		global $wpdb;
	 ...
	 * @return void
	 */
	public function update_noindex_values( $post_id, $noindex ) {
		global $wpdb;
		$wpdb->update(
			$wpdb->ahrefs_content,
			[ 'is_noindex' => $noindex ],
	 ...
	 * @return void
	 */
	public function update_position_values( $post_id, $position = null, $error = null ) {
		global $wpdb;
	 ...
	 * @return void
	 */
	public function post_positions_set_updated( array $post_ids ) {
		global $wpdb;
		Ahrefs_Seo::breadcrumbs( __METHOD__ . wp_json_encode( func_get_args() ) );
	 ...
	 */
	public function get_time_period_for_post( $post_id ) {
	 ...
	 * @return bool Success?
	 */
	private function get_time_period( $post_id = null, &$start_date, &$end_date, &$days_count ) {
		$waiting_time = Ahrefs_Seo_Data_Content::get()->get_waiting_weeks();
		if ( ! is_null( $post_id ) ) {
	 ...
	 * @return float
	 */
	private function traffic_median_calculate() {
		global $wpdb;
	 ...
	 */
	public function maybe_can_not_proceed() {
	...
	private function fill_with_errors( $no_ahrefs, $no_gsc, $no_ga ) {
		global $wpdb;
		Ahrefs_Seo::breadcrumbs( __METHOD__ . wp_json_encode( func_get_args() ) );
		if ( $no_ahrefs ) {
	 ...
	 * @return void
	 */
	private function maybe_update_snapshot_fields() {
	 ...
	 */
	protected function get_traffic_median() {
		$result = null;
		if ( ! is_null( $this->snapshot_id ) ) {
	 ...
	 * @return void
	 */
	private function content_reset_post_errors( $post_id, $reset_traffic_error = true, $reset_backlinks_error = true, $reset_position_error = true ) {
		global $wpdb;
		$updates = [];
		$format  = [];
	 ...
	 * @return void
	 */
	public function keyword_approve( $post_id, $approved_keyword ) {
		global $wpdb;
		if ( is_null( $this->snapshot_id ) ) {
	 ...
	 * @return bool Success?
	 */
	public function reanalyze_post( $post_id ) {
		$result         = false;
		$median_traffic = $this->get_traffic_median();
	 ...
	 */
	public function audit_include_posts( array $post_ids ) {
		global $wpdb;
			...
			if ( $this->snapshot->get_new_snapshot_id() === $this->snapshot_id && $this->snapshot->get_current_snapshot_id() !== $this->snapshot_id ) {
				...
				foreach ( $post_ids as $post_id ) {
					...
					if ( empty( $post ) || 'publish' !== $post->post_status ) {
						...
						$action = ! empty( $wpdb->get_var( $wpdb->prepare( "SELECT action FROM {$wpdb->ahrefs_content} WHERE snapshot_id = %d AND post_id = %d", $this->snapshot_id, $post_id ) ) ) ? $wpdb->get_var( $wpdb->prepare( "SELECT action FROM {$wpdb->ahrefs_content} WHERE snapshot_id = %d AND post_id = %d", $this->snapshot_id, $post_id ) ) : Ahrefs_Seo_Data_Content::ACTION4_ADDED_SINCE_LAST;
						switch ( $action ) {
	 ...
	 * @return void
	 */
	private function delete_not_published_post( $post_id ) {
		global $wpdb;
		$wpdb->delete(
			$wpdb->ahrefs_content,
			[
	 ...
	 */
	public function audit_exclude_posts( array $post_ids ) {
		global $wpdb;
		$errors_ids  = []; // result to return.
		$updated_ids = []; // will reset require_update flag of snapshot, if has some post here.
		if ( ! is_null( $this->snapshot_id ) ) {
	 ...
	 */
	protected function reset_all_fields( $post_id ) {
		global $wpdb;
	 ...
	 * @return void
	 */
	public static function audit_stop( array $messages, $scheduled_audit = null ) {
	 ...
	 * @return bool
	 */
	public static function audit_is_paused() {
	 ...
	 * @return Message[]|null
	 */
	public static function audit_get_paused_messages( $with_scheduled_audit = false ) {
		$results = [];
	 ...
	 * @return void
	 */
	public static function audit_clean_pause() {
	 ...
	 * @return void
	 */
	public static function audit_clean_scheduled_message() {
	 ...
	 */
	public static function audit_resume() {
	 ...
	 */
	public static function audit_is_delayed() {
