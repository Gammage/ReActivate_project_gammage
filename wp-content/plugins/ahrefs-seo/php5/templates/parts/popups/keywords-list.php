...
if ( ! is_null( $post ) ) {
	...
	<div class="ahrefs-seo-modal-keywords" id="ahrefs_seo_modal_keywords" data-id="<?php echo esc_attr( $locals['post_id'] ); ?>">
		<div class="keyword-header">Select target keyword for <a class="keyword-post-title" href="<?php echo esc_attr( (string) $post_url ); ?>">
																											 <?php
		...
		<script type="text/javascript">
			console.log('loaded');
			content.keyword_data_set = 
			<?php
			echo wp_json_encode( $data['keywords'] );
			?>
	;
			content.keyword_data_total_clicks = 
			<?php
			echo wp_json_encode( $data['total_clicks'] );
			?>
	;
			content.keyword_data_total_impr = 
			<?php
			echo wp_json_encode( $data['total_impr'] );
			?>
	;
			content.keyword_data_not_approved = 
			<?php
			echo wp_json_encode( ! $is_approved );
			?>
	;

			content.keyword_popup_update_table(); // initialize table with existing suggestions.
			content.keyword_popup_update_suggestions( 
			<?php
			echo wp_json_encode( $locals['post_id'] );
			?>
	 ); // run update for new suggestions.
		</script>
	</div>
	<?php
	...
	if ( ! is_null( $data['errors'] ) ) {
		...
		<script type="text/javascript">
			...
			<?php
			echo wp_json_encode( $data['errors'] );
			?>
		 );
		</script>
		<?php
