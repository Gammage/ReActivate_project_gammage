...
<div class="columns-wrap">
	<div class="one-column">
		<ul class="checkbox-group">
			<li>
				<input type="checkbox" id="posts_enabled" value="1" name="posts_enabled" class="checkbox-main" 
				<?php
				checked( ! $posts_disabled );
				?>
				>
				...
				<ul class="subitems">
				<?php
				wp_category_checklist( 0, 0, $posts_checked );
				?>
				</ul>
			</li>
		</ul>
	...
	<div class="one-column">
		<ul class="checkbox-group">
			<li>
				<input type="checkbox" id="pages_enabled" value="1" name="pages_enabled" class="checkbox-main" 
				<?php
				checked( ! $pages_disabled );
				?>
				>
		...
		>
								<label class="selectit">
									<input type="checkbox" name="pages[]" value="<?php echo esc_attr( "{$id}" ); ?>" id="<?php echo esc_attr( 'page_' . $id ); ?>" 
																							<?php
																							checked( in_array( "{$id}", $page_ids, true ) );
																							?>
		<label for="<?php echo esc_attr( 'page_' . $id ); ?>">
							<?php
							echo esc_html( $title );
							?>
		</label>
