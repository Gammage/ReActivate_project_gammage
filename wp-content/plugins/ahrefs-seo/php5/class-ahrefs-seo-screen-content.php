...
class Ahrefs_Seo_Screen_Content extends Ahrefs_Seo_Screen_With_Table {
	 ...
	 * @return void
	 */
	public function ajax_keyword_tip_close() {
		if ( check_ajax_referer( Ahrefs_Seo_Table::ACTION ) && current_user_can( Ahrefs_Seo::CAPABILITY ) ) {
	 ...
	 */
	public function bulk_actions( $doaction, array $ids ) {
		$sendback = [];
		switch ( $doaction ) {
			case 'trash':
				$trashed = 0;
	 ...
	 */
	protected function prepare_tips( $first_or_sub, $stop_displayed = '', $hide_compatibility = false ) {
		$result = [];
		...
		// audit-tip.
		ob_start();
		$tips = array_map( [ Message::class, 'create' ], ! empty( Ahrefs_Seo_Errors::get_saved_messages( null, 'tip' ) ) ? Ahrefs_Seo_Errors::get_saved_messages( null, 'tip' ) : [] );
		array_walk(
			$tips,
