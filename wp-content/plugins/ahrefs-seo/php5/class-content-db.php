...
class Content_Db {
	 ...
	 */
	public function content_get_organic_traffic_data_for_post( $post_id ) {
		global $wpdb;
		$snapshot_id = Ahrefs_Seo_Data_Content::get()->snapshot_context_get();
		$result      = $wpdb->get_row( $wpdb->prepare( "SELECT organic as organic_total, organic_month as organic_month FROM {$wpdb->ahrefs_content} WHERE post_id = %d AND snapshot_id = %d", $post_id, $snapshot_id ), ARRAY_A );
	...
	public function get_all_authors() {
		global $wpdb;
		$results = (array) $wpdb->get_results( "SELECT DISTINCT p.post_author as id, u.display_name as name FROM {$wpdb->posts} p JOIN {$wpdb->users} u ON p.post_author = u.ID WHERE p.post_status = 'publish' AND ( p.post_type IN ('post','page') ) LIMIT 1000", ARRAY_A );
		usort(
			$results,
			function ( $a, $b ) {
	 ...
	 * @return void
	 */
	public function reset_gsc_info( $snapshot_id ) {
		global $wpdb;
	 ...
	 * @return void
	 */
	public function reset_ga_info( $snapshot_id ) {
		global $wpdb;
	 ...
	 * @return void
	 */
	public function reset_backlinks_info( $snapshot_id ) {
		global $wpdb;
		// update all traffic and backlinks values to initial NULL.
