...
class Snapshot {
	...
	protected function fill_content_table( $new_snapshot_id ) {
		...
		if ( $posts_on ) {
			...
			$categories = ! empty( $content->get_posts_categories_checked() ) ? $content->get_posts_categories_checked() : [];
			$params     = [
				'post_type'   => 'post',
				'post_status' => 'publish',
			];
	 ...
	 */
	private function add_items_by_clause( $snapshot_id, array $params, $action ) {
		Ahrefs_Seo::breadcrumbs( __METHOD__ . wp_json_encode( func_get_args() ) );
		global $wpdb;
		$results                  = [];
		$paged                    = 1;
	 ...
	 * @return void
	 */
	public function analysis_approve_items( array $post_ids ) {
		$new_snapshot_id     = $this->get_new_snapshot_id();
		...
		foreach ( $post_ids as $post_id ) {
			...
			if ( ! is_null( $new_snapshot_id ) && $new_snapshot_id !== $current_snapshot_id ) {
				...
				$content_audit->keyword_approve( $post_id, ! empty( Ahrefs_Seo_Keywords::get()->post_keyword_get( $current_snapshot_id, $post_id ) ) ? Ahrefs_Seo_Keywords::get()->post_keyword_get( $current_snapshot_id, $post_id ) : '' );
			}
		}
	}
	/**
	 ...
	 * @return bool
	 */
	public function get_quick_update_allowed( $snapshot_id ) {
		global $wpdb;
	 ...
	 * @return void
	 */
	public function set_quick_update_allowed( $snapshot_id, $is_allowed ) {
		global $wpdb;
	 ...
	 * @return void
	 */
	public function set_finished( $new_snapshot_id ) {
		global $wpdb;
	 ...
	 * @return void
	 */
	public function add_post_as_added_since_last( $snapshot_id, $post_id ) {
		global $wpdb;
