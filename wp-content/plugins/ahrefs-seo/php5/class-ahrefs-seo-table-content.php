...
class Ahrefs_Seo_Table_Content extends Ahrefs_Seo_Table {
	...
	public function get_columns() {
		...
		$columns            = [
			...
			'date'       => 'Date',
			'action'     => 'excluded' !== $this->tab ? sprintf( '<span class="show_tooltip" title="" data-tooltip="Recommended based on the traffic, backlinks & waiting time thresholds you\'ve set in the <a href=\'%s\'>content audit settings.</a>">Suggestion</span>', esc_attr( $settings_content ) ) : '<span class="show_tooltip" title="" data-tooltip="The reason for post being excluded or not analyzed.">Reason</span>',
		];
		return $columns;
	}
	/**
	...
	protected function column_cb( $item ) {
		// Note: can not define type of parameters, because not defined in parent class.
		printf( '<input type="checkbox" name="link[]" value="%s" data-id="%d" data-ver="%d" />', absint( $item->post_id ), absint( $item->post_id ), intval( $item->ver ) );
	}
	/**
	...
	protected function column_action( \stdClass $item ) {
		...
		echo esc_html( $title );
		?>
		 <span src="<?php echo esc_attr( AHREFS_SEO_IMAGES_URL . 'arrow-down.svg' ); ?>" class="arrow-down"></span></a>
		<?php
	}
	/**
	 ...
	 * @param \stdClass $item Item.
	 */
	protected function column_more( \stdClass $item ) {
		?>
	 ...
	 * @return void
	 */
	protected function some_error_message() {
		?>
	 ...
	 * @return void
	 */
	protected function some_empty_message() {
		?>
		—
		<?php
	}
	...
	protected function keywords_dropdown_content() {
		...
		<?php
		__( 'Filter by keywords type' );
		?>
		</label>
		<select name="keywords" id="keywords" class="postform">
			<option value="" class="current"
			<?php
			selected( $this->keywords, '' );
			?>
		>All keywords</option>
			<option value="1" class="current"
			<?php
			selected( $this->keywords, '1' );
			?>
		>Approved by you</option>
			<option value="0" class="current"
			<?php
			selected( $this->keywords, '0' );
			?>
		>Suggested by the plugin</option>
			<option value="2" class="current"
			<?php
			selected( $this->keywords, '2' );
			?>
		>No keyword detected</option>
		</select>
		<?php
	}
	/**
