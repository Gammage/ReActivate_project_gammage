...
class Ahrefs_Seo_Advisor {
	 ...
	 */
	public function find_relevant_top_performing_pages( $snapshot_id, $post_id, $limit = 20 ) {
		$result = [];
	 ...
	 * @return null|array<string> Array ( post_id => title ) or null.
	 */
	public function find_relevant_under_performing_pages( $snapshot_id, $post_id, $limit = 20 ) {
		$result = [];
		$data   = $this->find_posts_with_same_keys( $snapshot_id, $post_id, [ Ahrefs_Seo_Data_Content::ACTION4_UPDATE_YELLOW, Ahrefs_Seo_Data_Content::ACTION4_MERGE, Ahrefs_Seo_Data_Content::ACTION4_EXCLUDE, Ahrefs_Seo_Data_Content::ACTION4_UPDATE_ORANGE, Ahrefs_Seo_Data_Content::ACTION4_DELETE ], $limit, false );
		if ( ! empty( $data ) ) {
			$data = array_flip( $data );
			foreach ( $data as $post_id => &$value ) {
				$value = get_the_title( $post_id );
	 ...
	 */
	public function find_active_pages_with_same_keyword( $snapshot_id, $post_id, $limit = 1000 ) {
		return $this->find_posts_with_same_keys( $snapshot_id, $post_id, [ Ahrefs_Seo_Data_Content::ACTION4_DO_NOTHING, Ahrefs_Seo_Data_Content::ACTION4_UPDATE_YELLOW, Ahrefs_Seo_Data_Content::ACTION4_MERGE, Ahrefs_Seo_Data_Content::ACTION4_EXCLUDE, Ahrefs_Seo_Data_Content::ACTION4_UPDATE_ORANGE, Ahrefs_Seo_Data_Content::ACTION4_DELETE, Ahrefs_Seo_Data_Content::ACTION4_ANALYZING_FINAL ], $limit, false );
	}
	/**
	 ...
	 * @return bool
	 */
	public function has_active_pages_with_same_keywords( $snapshot_id, $post_id ) {
	 ...
	 */
	private function find_posts_with_same_keys( $snapshot_id, $post_id, array $actions_filter = [], $limit = 100, $traffic_order_desc = true ) {
		global $wpdb;
		// post keywords.
		...
		usort(
			...
			function ( $a, $b ) use ( $traffic_order_desc ) {
				...
				return $traffic_order_desc ? intval( $b['traffic'] ) < intval( $a['traffic'] ) ? -1 : ( intval( $b['traffic'] ) == intval( $a['traffic'] ) ? 0 : 1 ) : ( intval( $a['traffic'] ) < intval( $b['traffic'] ) ? -1 : ( intval( $a['traffic'] ) == intval( $b['traffic'] ) ? 0 : 1 ) );
			}
		);
		return array_slice(
			array_map(
	 ...
	 */
	public function find_internal_links( $post_id, $limit = 1000 ) {
		global $wpdb;
		$result = [];
