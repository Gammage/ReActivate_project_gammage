...
class Keywords_Vectorizer {
	...
	public function __construct( Keywords_Tokenizer $tokenizer, $top_words_count = 50 ) {
	 ...
	 * @return void
	 */
	public function transform( array &$texts ) {
		array_walk(
			$texts,
			function ( &$text ) {
				if ( is_string( $text ) ) {
					$tokens = $this->tokenizer->tokenize( $text, self::WORDS_COUNT_MAX, true );
					$text   = $this->transform_counts( $tokens );
				} else {
					$text = [];
					Ahrefs_Seo::notify( new \Exception( sprintf( 'Transform: text parameter is not a string [%s]', wp_json_encode( $text ) ) ), 'Unexpected error on transform' );
				}
			}
		);
	}
	/**
	 * Default fast method.
	 * Get counts of prases and return recommended keywords using limit.
	 * Use stopwords from keyword tokenizer.
	 *
	 * @param array<int, array> $counts
	 *
	 * @return array<string, float> Associative array [ keyword => featured ], ordered by featured desc.
	 */
	protected function transform_counts( array &$counts ) {
		$sample = []; // reset sample values.
		if ( self::DECREASE_IF_HAS_STOP_WORD ) { // @phpstan-ignore-line -- constant can have any value in future.
				...
				if ( (int) $words_len > 1 ) {
					foreach ( $data as $phrase => &$count ) {
						...
						if ( count( $stop_words_count ) ) {
							// move phrase to lower words count. This will decrease it's importance.
		...
		$backup_words = [];
		if ( self::FALLBACK_TO_SHORT_PHRASE ) { // @phpstan-ignore-line -- constant can have any value in future.
			for ( $i = self::WORDS_COUNT_MIN; $i > 0; $i-- ) {
				if ( ! count( $backup_words ) && count( $counts[ $i ] ) ) {
					$backup_words = $counts[ $i ];
				}
			}
		}
		// filter by mininal words count.
		...
		if ( ! empty( $counts ) ) {
			 ...
			 * @var int[]
			 */
			$maximums  = array_map( 'max', $counts );
			$total_sum = array_sum( $sums ); // int.
			$count_sums = count( $sums ); // int.
			$max_words = count( $counts ); // int.
			$samples = [];
			// search best keywords here.
		...
		if ( ! count( $sample ) && self::FALLBACK_TO_SHORT_PHRASE ) { // @phpstan-ignore-line -- constant can have any value in future.
			arsort( $backup_words );
