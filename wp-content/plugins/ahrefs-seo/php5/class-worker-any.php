 ...
 * @since 0.7.3
 */
abstract class Worker_Any {

	const API_NAME             = '';
	const OPTION_PREFIX_WAIT   = 'ahrefs-seo-worker-wait';
	const OPTION_PREFIX_LOCK   = 'ahrefs-seo-worker-lock';
	const MAX_UPDATE_TIME_AJAX = 15;
	const MAX_UPDATE_TIME_CRON = 15;
	/**
	 * @var string One of 'traffic, 'backlinks', 'keywords', 'position', 'isnoindex'.
	 */
	const WHAT_TO_UPDATE = '';
	/**
	 * @var int Load up to (number) items in same request.
	 */
	protected $items_at_once = 5;
	/**
	 * Minimal delay after update_posts() failed. Real delay calculated in Worker_Any::on_rate_error() method.
	 *
	 * @var float
	 */
	 ...
	 * @param Ahrefs_Seo_Abstract_Api|null|mixed $api
	 * @param bool                               $is_cron Executed using cron.
	 */
	public function __construct( Content_Audit $content_audit, $api, $is_cron = false ) {
		$this->content_audit = $content_audit;
		$this->snapshot_id   = $content_audit->get_snapshot_id();
	...
	public function execute() {
		...
		try {
			Ahrefs_Seo::breadcrumbs( sprintf( '%s::update_posts: %s', get_called_class(), wp_json_encode( $items ) ) );
			try {
			...
			$error = Ahrefs_Seo_Compatibility::on_type_error( $e, __METHOD__, __FILE__ );
		} catch ( \Exception $e ) {
	...
	public function on_pause_now() {
		...
		return ! empty( $wait_until ) && (float) $wait_until > time();
	}
	/**
	 ...
	 * @return string
	 */
	protected function get_waiting_option_name() {
	 ...
	 * @return string
	 */
	protected function get_lock_name( $post_id ) {
		$class = str_replace( 'worker_', '', strtolower( str_replace( __NAMESPACE__ . '\\', '', get_class( $this ) ) ) );
		return sprintf( '%s%s-%d-%d', $this::OPTION_PREFIX_LOCK, $class, $this->snapshot_id, $post_id );
	}
	/**
	 * Save waiting time from API, to option.
	 * Do not overwrite current value if new time is smaller from existing.
	 *
	 * @param float $wait_seconds
	 * @return void
	 */
	protected function set_pause( $wait_seconds ) {
	 ...
	 * Get pause time.
	 *
	 * @param bool $cached_value
	 * @return float Timestamp in future or at the past, 0 if was not set.
	 */
	private function get_pause_value( $cached_value = false ) {
	 ...
	 * @return void
	 */
	private function lock_posts( array $post_ids ) {
		array_walk(
			$post_ids,
			function ( $post_id ) {
				set_transient( $this->get_lock_name( $post_id ), true, 5 * MINUTE_IN_SECONDS );
			}
		);
	}
	/**
	 ...
	 * @return void
	 */
	private function unlock_posts( array $post_ids ) {
		array_walk(
			$post_ids,
			function ( $post_id ) {
				delete_transient( $this->get_lock_name( $post_id ) );
			}
		);
	}
	/**
	 ...
	 * @return bool
	 */
	private function is_locked_post( $post_id ) {
		return (bool) get_transient( $this->get_lock_name( $post_id ) );
	}
	/**
	 ...
	 * Posts are locked.
	 *
	 * @return null|int[] List of post ID, null if can not return items, because something is blocked.
	 */
	protected function get_next_posts_wrapper() {
		$transient = $this->get_lock_name( 0 );
		$result    = null;
		$filled    = false;
		$n         = 0;
		do {
	 ...
	 * @return null|int[] List of post ID, null if can not return items, because something is blocked.
	 */
	protected function get_next_posts() {
		$data = $this->content_audit->get_unprocessed_item_from_new( $this::WHAT_TO_UPDATE );
		if ( ! empty( $data ) && is_array( $data ) ) {
			// need to filter using already locked posts.
			foreach ( $data as $key => $post_id ) {
				if ( $this->is_locked_post( (int) $post_id ) ) {
					unset( $data[ $key ] );
				}
			}
			if ( count( $data ) > $this->items_at_once ) {
				// leave only items_at_once items.
				$data = array_rand( array_flip( array_values( $data ) ), $this->items_at_once );
			}
			/** @var int[] */
	 ...
	 */
	abstract protected function update_posts( array $post_ids);
	/**
	 ...
	 * @return void
	 */
	abstract public function update_posts_info( array $post_ids);
	/**
	 ...
	 * @return void
	 */
	public function on_rate_error( $e, array $page_slugs_list = [] ) {
		$wait_minutes = null;
		Ahrefs_Seo::breadcrumbs( sprintf( '%s::%s(%s)(%s)', get_called_class(), __FUNCTION__, wp_json_encode( $page_slugs_list ), (string) $e ) );
		if ( $e instanceof InvalidArgumentException || $e instanceof GuzzleConnectException || $e instanceof Ahrefs_Seo_Exception ) {
			...
			$error        = isset( $e->getErrors()[0] ) ? $e->getErrors()[0] : null;
			if ( is_array( $error ) && isset( $error['reason'] ) ) {
				if ( in_array( $error['reason'], [ 'userRateLimitExceeded', 'rateLimitExceeded' ], true ) ) {
					$wait_minutes = 5;
				} elseif ( 'quotaExceeded' === $error['reason'] ) {
					$wait_minutes = 3;
	 ...
	 */
	public function get_waiting_seconds() {
	 ...
	 */
	public static function get_max_waiting_time() {
		$result = [];
