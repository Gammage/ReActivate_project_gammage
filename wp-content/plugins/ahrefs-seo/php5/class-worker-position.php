...
class Worker_Position extends Worker_GSC {
	 ...
	 * @return void
	 */
	public function update_posts_info( array $post_ids, $fast_update = false ) {
			...
			if ( ! empty( $post_with_keyword ) ) {
				...
				foreach ( $post_ids as $post_id ) {
					if ( isset( $post_to_url[ $post_id ] ) ) { // otherwise it is a skipped item or API returned no results for it.
						...
						if ( $by_post_id ) {
							$row = is_array( $data_all ) ? isset( $data_all[ $post_id ] ) ? $data_all[ $post_id ] : [] : [];
						} else {
							$row = is_array( $data_all ) ? isset( $data_all[ $url ] ) ? $data_all[ $url ] : [] : [];
						}
						$data                = isset( $row['pos'] ) ? $row['pos'] : null;
